...
class PPOLearner:
    ...
    def train(self, batch: EpisodeBatch, t_env: int, episode_num: int):
        # 确保训练前把 MAC 移到 GPU（runner 交互阶段可能把它挪回了 CPU）
        if self.args.use_cuda and str(self.mac.get_device()) == "cpu":
            self.mac.cuda()

        # 重新初始化隐藏状态到与模型一致的设备（避免 hidden_state 设备不匹配）
        self.mac.init_hidden(batch.batch_size)

        # Get the relevant quantities
        # 按 EpisodeBatch 的 in-place 语义迁移设备，避免覆盖为 None
        ...
        for _ in range(self.args.mini_epochs):
            ...
            for t in range(batch.max_seq_length-1):
                ...
                # if t == 0:
                #     try:
                #         print(f"[DEVDBG] PPO.train t={t} obs.device={batch['obs'][:, t].device} state.device={batch['state'][:, t].device}")
                #     except Exception as e:
                #         print(f"[DEVDBG] PPO.train t={t} device print failed: {e}")
                #     try:
                #         print(f"[DEVDBG] PPO.train mac_device={self.mac.get_device()}")
                #     except Exception:
                #         pass

                agent_outs = self.mac.forward(batch, t=t)
                pi.append(agent_outs)
            pi = th.stack(pi, dim=1)  # [bs, T, n_agents, n_actions]

            # Mask probs for unavailable actions without mutating original pi (numerical safety)
            masked_pi = pi.clone()
            masked_pi[avail_actions == 0] = 0.0
            # renormalize along actions; if a row sums to 0 (shouldn't normally), fallback to uniform
            sum_pi = masked_pi.sum(dim=-1, keepdim=True)
            uniform = th.full_like(masked_pi, 1.0 / masked_pi.size(-1))
            safe = sum_pi > 0
            masked_pi = th.where(safe, masked_pi / (sum_pi + 1e-12), uniform)

            pi_taken = th.gather(masked_pi, dim=3, index=actions)
            ...
            # entropy
            # Entropy on masked probs to avoid NaNs
            entropy_loss = categorical_entropy(masked_pi).mean(-1, keepdim=True) # mean over agents
            entropy_loss[mask == 0] = 0  # drop padded
            entropy_loss = (entropy_loss * mask).sum() / mask.sum()
            # total loss (avoid dividing by entropy.item which can be 0)
